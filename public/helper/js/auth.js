    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.alert === "success") {
                ...
                setTimeout(() => {
                    location.href = result.callback || '/login'; // Ganti dengan rute login jika tidak ada callback
                }, 4000);
            } else {
                Lobibox.notify('error', {
                    msg: result.message,
        ...
        error: function(xhr) {
            Lobibox.notify('error', {
                msg: xhr.responseJSON?.message || "Terjadi kesalahan. Silakan coba lagi.",
                position: 'top right',
            });
        }
    });
